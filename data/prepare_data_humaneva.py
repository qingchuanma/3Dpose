...
if __name__ == '__main__':
    ...
    if args.path:
        ...
        for subject in subjects:
            ...
            for f in file_list:
                ...
                #这段代码的目的是将原始数据按照预定义的帧范围和连续非 NaN 区间进行拆分，
                #保存到 output 和 output_2d 中，并记录每个区间的相关信息到 frame_mapping 中。
                # 这样可以在后续处理中根据需要使用这些信息。
        ...
        #metadata:{'layout_name': 'humaneva15', 'num_joints': 15, 'keypoints_symmetry': [[2, 3, 4, 8, 9, 10], [5, 6, 7, 11, 12, 13]]}
    ...
    if args.convert_2d:
        ...
        for f in file_list:
            #这段代码的主要目的是从文件列表中提取出主体、动作和相机的信息，并导入关键点数据，同时确保导入的关键点数据的关节数符合元数据的要求。
            ...
            if action in sync_data[subject]:
                #sync_data 是一个字典，其键是主体名称，对应的值是另一个字典，其键是动作名称，对应的值是一个元组，表示相机的同步偏移值
                sync_offset = sync_data[subject][action][camera_idx] - 1
            else:
                sync_offset = 0

#通过这段代码，将按照预定义的分块信息，从原始的关键点数据中提取出对应的数据块，
# 并按照规范组织保存在输出字典 output 中。这样做的目的是将原始数据按照动作和主体等信息进行切割，以方便后续处理和分析
